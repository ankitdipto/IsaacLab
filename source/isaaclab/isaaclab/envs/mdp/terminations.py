...
def joint_pos_out_of_limit(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # Get relevant joint positions and limits for specified joint IDs
    ...
    out_of_upper_limits = torch.any(joint_pos > soft_limits[..., 1], dim=1)
    out_of_lower_limits = torch.any(joint_pos < soft_limits[..., 0], dim=1)
